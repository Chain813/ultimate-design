...
| 编号 | 类型 | 说明 |
| --- | --- | --- |
| 🧩 `01-04` | 专业流程入口 | 对应城市设计 13 阶段的三大板块和现场调研，主要承载工作流的宏观导航 |
| 🧰 `11-15` | 功能模块页面 | 实际承载计算算法与分析结论的 5 个子系统页面 |

整个项目的物理路由与子功能模块构成如下树状结构：
...
│  ├─ 03_后期设计生成与成果表达.py
│  ├─ 04_现场调研.py               # 街景样本库
│  ├─ 11_数据底座与规划策略.py     # 包含：资产综合评估 / 策略语义萃取 / 物理底座管理
│  ├─ 12_现状空间全景诊断.py       # 包含：3D现状全息底座 / 地块级诊断面板
│  ├─ 13_AIGC设计推演.py           # 包含：地块导向推演 / 空间生形模式 / 约束参数 / 结果对比
│  ├─ 14_LLM博弈决策.py            # 包含：多主体五阶段推演 / 动态共识雷达 / 图纸提示词助手
│  └─ 15_更新设计成果展示.py       # 包含：更新设计大地图 / 规划文本成果 / 重点地块效果图
```
