...
def populate_c2_s1(p_next):
    ...
    insert_formatted_p(p, [
        ...
        ("[4]", False, False, True),
        ("，系统设立“居民-开发商-政府”三方智能体，模拟在利益协商中的角色对立，利用大语言模型进行三轮动态博弈协商（陈述、交锋、妥协），并通过文本语义与关键词命中双重机制评估满意度走势；", False, False, False)
    ])
    insert_formatted_p(p, [
...
def populate_c2_s2(p_next):
    ...
    insert_formatted_p(p, [
        ("系统的四大关键技术包括：一是「矢量-光栅-ControlNet」空间约束AI制图管线，将用地、道路等矢量红线转换为带国标色值的光栅图作为ControlNet of Stable Diffusion的输入，彻底消除了生成式AI在规划底图上的空间位置漂移与形态幻觉；二是Data-to-Text大模型动态图例说明合成技术，在A3底板封装阶段，大模型（deepseek-v4-flash）读取GIS实测指标，自动生成设计说明文字，确保出图指标与真实GIS物理数据严格绑定；三是大语言模型（LLM）多主体博弈协商与社会共识决策技术，建立由大模型驱动的“居民-开发商-政府”多智能体协同机制，在多轮交锋与妥协中自动拟合各方诉求，达成带有法规依据与空间落位的更新策略；四是Zoning RAG 控规合规自动比对审计，通过大语言模型检索本地规范文本，在Streamlit前端对超出红线的违规指标高亮违规警告。", False, False, False)
    ])
...
def populate_c5_s1(p_next):
    ...
    insert_formatted_p(p, [
        ...
        (" 针对体检中暴露的高度和绿地问题，系统在设计策略页面模拟了三方利益博弈协商。开发商为了高投资回报，强烈要求将限高放宽至36m，在对话流中多次提及“投资回报、文旅产业”，满意度冲至80分，但规划局坚守“历史保护区、紫线控制、合规条例”底线（核心区≤9m，一般区≤18m），满意度为65分，居民代表则强烈反对高层压迫，要求增加口袋公园和生活配套，满意度得分仅为45分。由于 min(", False, False, False),
        ("S_res", False, True, False),
        (", ", False, False, False),
        ("S_dev", False, True, False),
        (", ", False, False, False),
        ("S_gov", False, True, False),
        (") = 45 < 60，触发利益冲突警报。在此警告下，系统引导开发商退让，调整方案将历史核心区限高严格控制在9m以内，一般区控制在18m以内，并在外围站前区兼容商办，同时配套建设3个口袋公园。经过第三轮妥协共识协商，三方满意度得分提升至66分、73分和68分，全部越过60分共识线，系统成功达成更新策略共识。", False, False, False)
    ])
    insert_formatted_p(p, [
