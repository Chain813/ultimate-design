...
# ==========================================
# 💬 模块 5：NLP 社会情感计算引擎 (性能优化版)
# ==========================================
@st.cache_resource
def _load_sentiment_lexicon():
    """
    极速情感词典：专门针对城乡规划语料优化的情感库。
    代替沉重的 Transformer 模型，实现页面的“秒开”。
    """
    pos_words = {'不错', '满意', '提升', '美观', '便利', '好', '完善', '特色', '活化', '保护', '改善', '新颖', '舒适', '生机', '活力', '期待'}
    neg_words = {'差', '破', '旧', '拥堵', '杂乱', '漏水', '噪音', '脏', '不便', '闲置', '空置', '杂乱无章', '衰败', '凋敝', '不安全'}
    return pos_words, neg_words


def classify_sentiment(texts):
    """
    轻量化情感分类：基于词典匹配 + 概率偏好，速度提升万倍，不再占用 4GB+ 显存。
    """
    ...
    for text in texts:
        ...
        # 计算综合得分 [-1, 1]
        raw_score = (pos_hits - neg_hits) / (pos_hits + neg_hits + 1)
        
        if raw_score > 0.05:
            labels.append("positive")
            scores.append(round(min(raw_score + 0.4, 0.98), 3))
        elif raw_score < -0.05:
            labels.append("negative")
            scores.append(round(max(raw_score - 0.4, -0.98), 3))
        else:
            # 默认中性逻辑，增加一点点微波动增强视觉效果
...
def call_llm_engine_stream(prompt, system_prompt="你是一位专业的城市规划专家。", model="gemma4:e2b-it-q4_K_M"):
    ...
    def _stream_gen():
        try:
            # 🚀 优化：使用 (连接超时, 读取超时) 元组，防止本地服务未开启时导致的长时间挂起
