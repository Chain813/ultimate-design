...
def get_or_generate_satellite_image(STATIC_DIR):
    # Depend on DR-122
    filename = "DR-122_清禾集贸市场-现状卫星图.png"
    drawing_type = "清禾集贸市场-现状卫星图"
    code = "DR-122"
    out_path = STATIC_DIR / "atlas" / filename
    
    if not out_path.exists():
        print(f"[DR-132] Dependency satellite sheet {filename} not found, generating it now...")
...
def get_layout_image(STATIC_DIR):
    # Search for DR-127_清禾集贸市场-改造总平面图 in atlas first, then backup
    fuzzy_name = "清禾集贸市场-改造总平面图"
    ...
    raise FileNotFoundError(f"[DR-132] Cannot find layout plan image for {fuzzy_name}")

def draw_map_early(output_path, view_w, view_h, STATIC_DIR):
    print("Drawing DR-132 Before-and-After Comparison Sheet...")
    ...
    try:
        ...
        print(f"[DR-132] Failed to load satellite image: {e}. Using empty canvas.")
        sat_map = Image.new("RGB", (1705, 1369), color=(240, 240, 240))
        
    try:
        layout_map = get_layout_image(STATIC_DIR)
    except Exception as e:
        print(f"[DR-132] Failed to load layout image: {e}. Using empty canvas.")
    ...
    draw.text((55, 90), "清禾集贸市场地块 — 改造前后对比图", fill=(15, 23, 42), font=font_large_title, anchor="lm")
    ...
    design_notes = [
        "规划方案：清禾集贸市场地块以“历史界面缝合者”为理念。缝合铁路与主干道带来的空间割裂，梳理人行游线，重构开放连通的城市公共交往节点。",
        "1. 界面缝合：通过跨铁路人行桥与地下通道梳理，缝合铁路割裂带，改善沿铁路线空间消极绿化与硬质风貌。",
        "2. 交通整合：重构出入口人车组织，增设自行车与电瓶车换乘及停放区，打通站前与生活区步行瓶颈。",
        "3. 空间微更新：置换低效库房大棚，注入露天文化集市、社区休闲袖珍节点，并利用绿化墙实施风貌织补。"
    ]
    y_pos = 1150
    ...
    print(f"[DR-132] Comparison drawing generated successfully: {output_path}")
    return view_w
