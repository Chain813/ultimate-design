...
def get_or_generate_satellite_image(STATIC_DIR):
    # Depend on DR-140
    filename = "DR-140_中国石油-现状卫星图.png"
    drawing_type = "中国石油-现状卫星图"
    code = "DR-140"
    out_path = STATIC_DIR / "atlas" / filename
    
    if not out_path.exists():
        print(f"[DR-150] Dependency satellite sheet {filename} not found, generating it now...")
...
def get_layout_image(STATIC_DIR):
    # Search for DR-145_中国石油-改造总平面图 in atlas first, then backup
    fuzzy_name = "中国石油-改造总平面图"
    ...
    raise FileNotFoundError(f"[DR-150] Cannot find layout plan image for {fuzzy_name}")

def draw_map_early(output_path, view_w, view_h, STATIC_DIR):
    print("Drawing DR-150 Before-and-After Comparison Sheet...")
    ...
    try:
        ...
        print(f"[DR-150] Failed to load satellite image: {e}. Using empty canvas.")
        sat_map = Image.new("RGB", (1705, 1369), color=(240, 240, 240))
        
    try:
        layout_map = get_layout_image(STATIC_DIR)
    except Exception as e:
        print(f"[DR-150] Failed to load layout image: {e}. Using empty canvas.")
    ...
    draw.text((55, 90), "中国石油地块 — 改造前后对比图", fill=(15, 23, 42), font=font_large_title, anchor="lm")
    ...
    design_notes = [
        "规划方案：中国石油地块设计为“宽城子能量花园”。重点整治加油站周边低效闲置零星空地，植入绿色生态缓冲屏障与运动空间，改善滨河风貌。",
        "1. 缓冲屏障：在加油站与生活区、滨河区之间植入宽大厚实的绿色生态缓冲林，屏蔽噪声与废气污染。",
        "2. 活力置换：清理沿河消极闲置零星空地，植入全龄运动场地、林荫游憩步道与滨河悬挑景观平台。",
        "3. 韧性修复：完全保留自然生态岸线，通过海绵城市绿地蓄排设计构建雨水花园，打造纯生态滨河公园。"
    ]
    y_pos = 1150
    ...
    print(f"[DR-150] Comparison drawing generated successfully: {output_path}")
    return view_w
