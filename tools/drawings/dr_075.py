...
def get_or_generate_satellite_image(STATIC_DIR):
    # Depend on DR-062
    filename = "DR-062_老水产市场-现状卫星图.png"
    drawing_type = "老水产市场-现状卫星图"
    code = "DR-062"
    out_path = STATIC_DIR / "atlas" / filename
    
    if not out_path.exists():
        print(f"[DR-075] Dependency satellite sheet {filename} not found, generating it now...")
...
def get_layout_image(STATIC_DIR):
    # Search for DR-067_老水产市场-改造总平面图 in atlas first, then backup
    fuzzy_name = "老水产市场-改造总平面图"
    ...
    raise FileNotFoundError(f"[DR-075] Cannot find layout plan image for {fuzzy_name}")

def draw_map_early(output_path, view_w, view_h, STATIC_DIR):
    print("Drawing DR-075 Before-and-After Comparison Sheet...")
    ...
    try:
        ...
        print(f"[DR-075] Failed to load satellite image: {e}. Using empty canvas.")
        sat_map = Image.new("RGB", (1705, 1369), color=(240, 240, 240))
        
    try:
        layout_map = get_layout_image(STATIC_DIR)
    except Exception as e:
        print(f"[DR-075] Failed to load layout image: {e}. Using empty canvas.")
    ...
    draw.text((55, 90), "老水产市场地块 — 改造前后对比图", fill=(15, 23, 42), font=font_large_title, anchor="lm")
    ...
    design_notes = [
        "规划方案：老水产批发市场地块改造以“文创街区”为核心。通过对低效水产市场进行微改造，注入艺术零售、创意市集和口袋公园等，提升空间活力。",
        "1. 功能活化：保留原有大跨度大棚肌理，注入文创办公、手作零售与艺术展览等业态，打造具有烟火气的文创活力街区。",
        "2. 空间织补：增设袖珍绿地与口袋公园，打通地块内部微循环，缝合与周边街区及东侧伊通河的绿色通道。",
        "3. 设施提升：增设社区级公共交往节点、智慧导引与便利设施，全面补齐老旧小区及周边配套短板。"
    ]
    y_pos = 1150
    for i, note in enumerate(design_notes):
        # bold the first line (general brief)
    ...
    print(f"[DR-075] Comparison drawing generated successfully: {output_path}")
    return view_w
