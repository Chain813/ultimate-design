...
def get_or_generate_satellite_image(STATIC_DIR):
    # Depend on DR-083
    filename = "DR-083_食品调料市场-现状卫星图.png"
    drawing_type = "食品调料市场-现状卫星图"
    code = "DR-083"
    out_path = STATIC_DIR / "atlas" / filename
    
    if not out_path.exists():
        print(f"[DR-095] Dependency satellite sheet {filename} not found, generating it now...")
...
def get_layout_image(STATIC_DIR):
    # Search for DR-088_食品调料市场-改造总平面图 in atlas first, then backup
    fuzzy_name = "食品调料市场-改造总平面图"
    ...
    raise FileNotFoundError(f"[DR-095] Cannot find layout plan image for {fuzzy_name}")

def draw_map_early(output_path, view_w, view_h, STATIC_DIR):
    print("Drawing DR-095 Before-and-After Comparison Sheet...")
    ...
    try:
        ...
        print(f"[DR-095] Failed to load satellite image: {e}. Using empty canvas.")
        sat_map = Image.new("RGB", (1705, 1369), color=(240, 240, 240))
        
    try:
        layout_map = get_layout_image(STATIC_DIR)
    except Exception as e:
        print(f"[DR-095] Failed to load layout image: {e}. Using empty canvas.")
    ...
    draw.text((55, 90), "食品调料市场地块 — 改造前后对比图", fill=(15, 23, 42), font=font_large_title, anchor="lm")
    ...
    design_notes = [
        "规划方案：食品调料大市场地块体量庞大，规划聚焦于“活态市集与风味院落”。整合工业厂房与调料大棚遗存，形成独具风味的历史文创消费街区。",
        "1. 结构重塑：采用“一核两带三组团”的空间结构，保留整合大棚工业遗存，形成活态市集与中心广场。",
        "2. 空间织补：拆除部分低效搭建，植入风味院落、街区口袋公园与雨水花园，绿地率大幅提升至35%以上。",
        "3. 交通梳理：打通内部微循环路网，梳理慢行通勤游线，建立人车分流的高品质特色消费空间。"
    ]
    y_pos = 1150
    ...
    print(f"[DR-095] Comparison drawing generated successfully: {output_path}")
    return view_w
