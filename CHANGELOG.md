...
* **研究范围地图 Banner 裁剪与容器贴合优化**：
  * **解决显示不全**：将系统主页头部 Banner 右侧地图图片的缩放属性从 `object-fit: cover` 优化为 `object-fit: contain`，彻底解决了地图底部（包含关键的图例说明和指北针）被强行裁剪的问题。
  * **图框完美贴合**：通过 Python 读取识别出该底图的物理尺寸为 `2500x2500`（1:1 正方形），将 CSS 布局属性从 `flex: 1.8` 升级为 **`aspect-ratio: 1 / 1`** 正方形比例约束，使带发光渐变的 3D 图框容器与内部的地图完美贴合，**彻底消除了两侧原本存在的黑底空白边缘**，整体质感极为精致高级。
