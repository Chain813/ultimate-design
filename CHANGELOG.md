...
* **研究范围地图 Banner 裁剪与容器贴合优化**：
  ...
  * **定位与 3D 透视偏移矫正**：针对 3D 旋转（`rotateY(-25deg)`）导致的视觉左移和右侧边界留白，引入了 **`transform-origin: right center`**，并优化了左右边距（`margin-left: 40px; margin-right: -10px;`），使图框容器完美贴靠在 Banner 的最右侧，完全对齐系统的网格布局线。
