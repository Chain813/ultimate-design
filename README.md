...
UltimateDESIGN 是面向城乡规划专业毕业设计与城市设计课程的 **Streamlit 全栈决策支持平台**。项目以长春伪满皇宫周边 170.2 公顷街区为实证对象，将城市设计拆解为 16 个标准化阶段（数据准备 → 现状诊断 → 概念策略 → 总体与专项设计 → 成果表达与交付 → AIGC推演与智能体技能手册），打通「GIS 数据采集 → LLM 循证推演 → AIGC 标准制图 → 智能化交付与图则」的完整闭环。
