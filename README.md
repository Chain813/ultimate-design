...
- **端到端 AIGC 渲染管线与质量闭环**
  内置 `SDPipeline` 统一渲染引擎（支持 txt2img、img2img、inpainting、高清放大及多 ControlNet 叠加）和 `DrawingPipeline` 端到端编排器，实现「一键出图」与「批量生成」。集成 `QualityAssessor` 双重质量评估（Ollama Gemma 视觉 + DeepSeek 内容），对 C/D 级结果自动修正提示词并重新生成，确保图册输出质量。
- **版本管理与图册级批量导出**
