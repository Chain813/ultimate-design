...
| **重点更新单元** | Dynamic Key Plot | 平台重点深化设计的最小空间对象，由 `Key_Plots_District.json` / GeoJSON 当前记录动态决定数量，不再绑定固定 5 个地块。Stage 10、Stage 13、诊断卡片、提示词与图纸目录均按该清单自动展开。 |
| **重点地块 GIS 处理** | Key Plot GIS Processing | `scripts/process_key_plots.py` 提供的空间数据清洗管线，包含 CRS 归一、无效几何修复、研究范围裁切、投影/测地面积测算、字段标准化与 WGS84 GeoJSON 输出。 |
...
| **多版式图纸引擎** | Multi-Layout Sheet Engine | `drawing_layout_engine.py` 提供的 A3 排版配置系统，内置图例右栏、双图对比、分析仪表盘、矩阵分镜、满版效果图、章节封面等版式，并将版式约束同步传递给提示词与图框生成链路。 |
